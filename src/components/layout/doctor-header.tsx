...
import { DOCTOR_NAV_ITEMS, isRouteActive } from "./doctor-nav-config";
...
/**
 * Responsive Header for the Doctor Portal
 */
export function DoctorHeader({
  ...
  const currentSection = React.useMemo(() => {
    const matched = DOCTOR_NAV_ITEMS.find((item) =>
      isRouteActive(pathname, item.href)
    );
    return matched ? t(matched.labelKey) : t("doctors.portalTitle");
  }, [pathname, t]);
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-border bg-card/95 backdrop-blur-md px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4 transition-colors">
      ...
      {/* End Actions: Language Switcher & Doctor Actions */}
